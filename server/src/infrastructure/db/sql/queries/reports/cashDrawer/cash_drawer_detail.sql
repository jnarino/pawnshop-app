-- Get all transactions for the requested date range
-- Returns all transaction and tender records - use case handles grouping and balance calculation
SELECT
...
LEFT JOIN store_transaction_tender AS stt
  ON stt.store_transaction_id = st.id
LEFT JOIN tender_type AS ttype
  ON ttype.id = stt.tender_type_id
LEFT JOIN pawn_ticket AS pt
  ON sttype.code = 'PPU'
 AND pt.control_number = st.legacy_ticketnum
WHERE st.occurred_at >= date_trunc('day', $1::timestamptz)
