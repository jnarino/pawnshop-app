...
import { generateGunTransactionHistoryPdf } from "./generateGunTransactionHistoryPdf";
import { AlertModal } from '@/app/shared/components/AlertModal';

export const GunTransactionHistoryReportPage = () => {
    ...
    const submit = async (data: { dateRange: { from: string; to: string } }) => {
        ...
        try {
            ...
            if (from && to) {
                const result = await reportsApi.gunTransactionHistoryReport({
                ...
                if (!result || result.length === 0) {
                    setAlertMessage("No gun transaction records found.");
                    return;
                }

                const pdfBlob = await generateGunTransactionHistoryPdf(result, { from, to });
    ...
    const handleDownload = () => {
        ...
        link.setAttribute(
            'download',
            `GunTransactionHistory_${from.split('T')[0]}_${to.split('T')[0]}.pdf`,
        );
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-100px)] pt-4 space-y-4">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-extrabold">Gun Transaction History Report</h1>
